...
/**
 * @file    eden_model.h
 * @brief   This is EDEN Model class
 * @details This header defines EDEN Model class.
 *          This class is implementing the Eden Model.
...
typedef enum {
  MODEL_STATE_NOT_READY = 0,        //*< Model does not have enough information to represent complete model.
  MODEL_STATE_READY,                //*< Model has enough information to represent complete model.
...
class EdenModel {
 public:
  ...
  /**
   * @brief Add operand for options into a model and return a operandId representing for it
   * @details This function adds new operand specified by a operandInfo.
   *          Once the operand is added on model,
   *          a unique operand id is mapped to that operand.
   * @param[in] operandInfo Information for operand to be added.
   ...
   * @returns return code
   */
  NnRet SetOperandValue(int32_t operandId, void* bufferAddr, int32_t offset, int32_t length);

  /**
   * @brief Add new operation for opType to model
   * @details This function adds new operation for opType.
   *          Caller should set correct input/output operand information as per current opType.
   ...
   * @details This function sets input/output operands for this model with a given information.
   ...
   * @returns return code
   */
  NnRet IdentifyInputsAndOutputs(int32_t numOfInputs, int32_t* inputOperandIndexes,
                                 int32_t numOfOutputs, int32_t* outputOperandIndexes);

   /**
   * @brief Identify output operands of NCP for this model.
   * @details This function sets output operands for this model with a given information.
   * @param[in] numOfOutputs Number of item on outputOperandIndexes.
   * @param[in] outputOperandIndexes Array of operands for output.
   * @returns return code
   */
  NnRet SetNcpOutputIndexes(int32_t numOfOutputs, int32_t* outputOperandIndexes);

  /**
   * @brief Identify output bridge operands for this model.
   * @details This function sets output bridge operands for this model with a given information.
   * @param[in] numOfOutputs Number of item on outputOperandIndexes.
   * @param[in] outputOperandIndexes Array of bridge operands for output.
   * @returns return code
   */
  NnRet SetBridgeIndexes(int32_t numOfOutputs, int32_t* outputOperandIndexes);
   ...
   * @details This function creates a EdenShapeInfo with a given parameters and sets it to
   ...
   * @returns return code
   */
  NnRet SetOperandShapeInfo(int32_t operandId, EdenDataType dataType, EdenBufferLayout bufferLayout,
                            int32_t numOfContents, int32_t channel, int32_t height, int32_t width);

  /**
   * @brief Set EdenScaleQuantInfo for a EdenOperand of operandId
   * @details This function creates a EdenScaleQuantInfo with a given parameters and sets it to
   *          the EdenOperand of operandId.
   * @param[in] operandId
   * @param[in] scale
   * @param[in] zeroPoint
   ...
   * @returns return code
   */
  NnRet SetOperandScaleQuantInfo(int32_t operandId, float scale, uint32_t zeroPoint, float realValueMin, float realValueMax, uint32_t bitLength);

  /**
   * @brief Set EdenFLQuantInfo for a EdenOperand of operandId
   * @details This function creates a EdenFLQuantInfo with a given parameters and sets it to
   *          the EdenOperand of operandId.
   * @param[in] operandId
   ...
   * @returns return code
   */
  NnRet SetOperandFLQuantInfo(int32_t operandId, uint32_t integerBitLength, uint32_t fractionalBitLength, uint32_t bitLength);
  ...
  void SetMemoryManager(EdenMemoryManager* memoryManager) { memoryManager_ = memoryManager; }
  void SetPrivateData(std::unique_ptr<int8_t[]>& privateData) { privateData_ = std::move(privateData); }
  void SetModelPreference(ModelPreference pref) { modelPreference_ = pref; }
  void SetComputePrecision(ComputePrecision precision) { computePrecision_ = precision; }
  void SetTileSize(int32_t tileSize) { tileSize_ = tileSize; }

  void SetDataTailorPrecisionAndType_pre(EdenDataType dataType, EdenDataTypeSize dataTypeSize) {
      dataTailorPrecision_preProcess = dataType;
      dataTailorPrecisionSize_preProcess = dataTypeSize;
  }
  void SetDataTailorPrecisionAndType_post(EdenDataType dataType, EdenDataTypeSize dataTypeSize) {
      dataTailorPrecision_postProcess = dataType;
      dataTailorPrecisionSize_postProcess = dataTypeSize;
  }

  void AddModelCompileData(std::string data) { modelCompileData_.push_back(data); }
  void NpuIsNotSupportingSoftmax(void) { npuIsNotSupportingSoftmax_ = true; }
  void ClearNpuIsNotSupportingSoftmax(void) { npuIsNotSupportingSoftmax_ = false; }
  void SetCellAlign(bool cellAlign);
  void SetPresetScenarioId(const int32_t& presetScenarioId) { presetScenarioId_ = presetScenarioId; }
  ...
  int32_t GetTileSize(void) { return tileSize_; }

  void GetDataTailorPrecisionAndType_pre(EdenDataType& type, EdenDataTypeSize& size) {
    type = dataTailorPrecision_preProcess;
    size = dataTailorPrecisionSize_preProcess;
  }
  void GetDataTailorPrecisionAndType_post(EdenDataType& type, EdenDataTypeSize& size) {
    type = dataTailorPrecision_postProcess;
    size = dataTailorPrecisionSize_postProcess;
  }
  ...
  uint32_t GetCellAlignSizeChannel(void) const { return cellAlignSize_.channel; }
  uint32_t GetCellAlignSizeNumber(void) const { return cellAlignSize_.number; }

  bool& ShouldRunCFU(void) { return shouldRunCFU; }
  bool& isCallbackSkippedForReserved(void) { return callbackSkipped; }
  bool& GetPrePostProcessPerf(void) { return prePostPorcessPerf; }
  bool& useNonCacheableInputBuffer(void) { return allocNonCacheableInputBuffer; }
  bool& isBindingOutput(void) { return bindingOutput; }
  bool& isBindingBridgeBuffer(void) { return bindingBridgeBuffer; }
  bool& isNcpTempBuffer(void) {return ncpTempBuffer; }
  // Async Call of NPU UD and DSP UD, Scenario Test Cleanup by removing
  // older failing test
  bool& AsyncRunNpuDsp(void) { return asyncRunNpuDsp; }

  // Shared Memory Operand Idx Getter, Setter.
  void SetSharedMemOperand(int32_t opIdx) { sharedMemIndex_ = opIdx; }
  int32_t GetSharedMemOperandIndex() { return sharedMemIndex_; }

  std::vector<EdenOperand*> const & GetOperands() const { return operands_; }
  std::vector<EdenOperand*> const & GetOperandsForOption() const { return operandsForOptions_; }
  std::vector<EdenOperation*> const & GetOperations() const { return operations_; }
  std::vector<int32_t> const & GetInputIndexes() const { return inputIndexes_; }
  std::vector<int32_t> const & GetOutputIndexes() const { return outputIndexes_; }
  std::vector<int32_t> const & GetNCPOutputIndexes() const { return ncpOutputIndexes_; }
  std::vector<int32_t> const & GetBridgeIndexes() const { return bridgeIndexes_; }

  std::map<int32_t, EdenOperand*> const & GetMapToOperand(void) const { return mapToOperand_; }
  std::map<int32_t, EdenOperation*> const & GetMapToOperation(void) const { return mapToOperation_; }
  std::map<EdenOperand*, int32_t> const & GetMapToOperandId(void) const { return mapToOperandId_; }
  std::map<EdenOperation*, int32_t> GetMapToOperationId(void) const { return mapToOperationId_; }

  std::map<int32_t, EdenNode*> const & GetMapToNode(void) const { return mapToNode_; }

  std::unordered_multimap<int32_t, EdenOperation*> const & GetMapInputOperandIdToOperation(void) const { return mapInputOperandIdToOperation_; }
  std::unordered_multimap<int32_t, EdenOperation*> const & GetMapOutputOperandIdToOperation(void) const { return mapOutputOperandIdToOperation_; }
  ...
  EdenOperation* GetOneEdenOperationHavingEdenOperandAsOutput(int32_t operandId);
  ...
  int32_t const & GetPresetScenarioId(void) const { return presetScenarioId_; }
  ...
  bool UpdateOperandDimensions(int32_t operandIndex, int32_t numOfDims, int32_t* newDims);
 ...
 private:
  ...
  /**
   * @brief Add new operation for opType to model
   * @details This function adds new operation for opType but sets a given operation id
   *          Caller should set correct input/output operand information as per current opType.
   ...
   * @details This function creates a graph representation with current model information.
   *          The operand and operation information is used.
   *          If model information is not ready to create a complete DAG, error is returned.
   * @todo Need to introduce internal state machine to reconginze model information is ready.
   * @param void
   * @returns return code
   */
  ...
  int32_t state_;     //*< internal state for this model (e.g. ready to executed, not yet identified for input/output etc)
  int32_t errno_;
  int32_t prepare_;

  ModelPreference modelPreference_;
  ComputePrecision computePrecision_;
  EdenDataType dataTailorPrecision_preProcess;
  EdenDataType dataTailorPrecision_postProcess;
  EdenDataTypeSize dataTailorPrecisionSize_preProcess;
  EdenDataTypeSize dataTailorPrecisionSize_postProcess;
  uint32_t numberOfRequest_;

  std::vector<EdenOperand*> operands_;            //*< Array of EdenOperand
  std::vector<EdenOperand*> operandsForOptions_;  //*< Array of EdenOperand used for XXXOptions
  ...
  std::vector<int32_t> ncpOutputIndexes_;
  std::vector<int32_t> bridgeIndexes_;
  int32_t nextOperandId_;
  int32_t nextOperationId_;

  std::map<int32_t, EdenOperand*> mapToOperand_;        //*< Map for (operandId, EdenOperand*)
  std::map<int32_t, EdenOperation*> mapToOperation_;    //*< Map for (operationId, EdenOperation*)
  std::map<EdenOperand*, int32_t> mapToOperandId_;      //*< Map for (EdenOperand*, operandId)
  std::map<EdenOperation*, int32_t> mapToOperationId_;  //*< Map for (EdenOperation*, operationId)

  std::unordered_multimap<int32_t, EdenOperation*> mapInputOperandIdToOperation_;   //*< Map for (operandId for input, EdenOpeartion)
  std::unordered_multimap<int32_t, EdenOperation*> mapOutputOperandIdToOperation_;  //*< Map for (operandId for output, EdenOpeartion)

  EdenNode* graphRoot;                //*!< Root for graph representation
  std::map<int32_t, EdenNode*> mapToNode_;  //*< Map for (operationId, EdenNode*)
  ...
  bool shouldRunCFU;
  bool callbackSkipped;
  bool prePostPorcessPerf;
  bool allocNonCacheableInputBuffer;  //*< Let EdenManager::allocateInputBuffers() allocate ION without cacheable flag
  bool bindingOutput;
  bool bindingBridgeBuffer;
  bool ncpTempBuffer;   //* To check whether Ncp Temp Buffers are required or not

  bool asyncRunNpuDsp;
  int32_t sharedMemIndex_;  //*! < Shared Memory Operand Idx to allocate Shared Memory used between NPU and DSP.
  bool npuIsNotSupportingSoftmax_;
  CellAlignSize cellAlignSize_;
  int32_t tileSize_;
  int32_t presetScenarioId_;
};
