...
typedef enum {
  P0 = 0, P1, P2, P3, P4, P_DEFAULT, P6, P7, P8, P9, P10
} ModelPriority;
...
struct EdenModelOptions {
  ...
  int32_t reserved[30];
  EdenModelOptions(): modelPreference{}, priority(P_DEFAULT), latency(0), boundCore(BOUND_NA), tileSize(1),
                      presetScenarioId(PRESET_DISABLE_UPPER_BOUND), reserved{} {}
};
#else
...
typedef struct __EdenModelFile {
  ...
  int8_t* pathToWeightBiasFile;       /*!< path to weight & bias file ending with "\0" */
...
typedef void (*NotifyFuncPtr) (addr_t*, addr_t);                  /*!< function pointer for notify */
typedef int32_t (*WaitForFuncPtr) (addr_t*, uint32_t, uint32_t);  /*!< function pointer for wait */
