...
/**
 * @file    EdenRuntime.h
 * @brief   This is EDEN Runtime API
 * @details This header defines EDEN Runtime API.
 * @version 0.1 Basic scenario support.
 * @version 0.2 Basic scenario support & modified
 *          Supported functions are as below.
 *          RtRet Init(void)
 *          RtRet OpenModel(EdenModel* model, uint32_t* modelId, EdenPreference preference)
 *          RtRet ExecuteReq(EdenReq* req, EdenEvent** evt, EdenPreference preference)
 *          RtRet CloseModel(uint32_t modelId)
 *          RtRet Shutdown(void)
 * @version 0.4 Multiple Requests support
 * @version 0.6 Multiple Open Models support
 ...
 */
RtRet OpenModel(eden::nn::EdenModel* model, uint32_t* modelId, ModelPreference preference);

/**
 *  @brief Open EDEN Model
 *  @details This API function set a unique ID to EdenModel, options, and compute the running path.
 ...
 *  @returns return code
 */
RtRet OpenModel(eden::nn::EdenModel* model, uint32_t* modelId, const EdenModelOptions& options);
...
/**
 *  @brief [Deprecated] Read a in-memory model on address and open it as a EdenModel
 *  @details Deprecated function.
 */
RtRet OpenModelFromMemory(ModelTypeInMemory modelTypeInMemory, int8_t* addr, int32_t size, bool encrypted,
                          uint32_t* modelId, ModelPreference preference);
 ...
 *  @returns return code
 */
RtRet AllocateInputBuffers(uint32_t modelId, EdenBuffer** buffers, int32_t* numOfBuffers);
 ...
 *  @returns return code
 */
RtRet AllocateOutputBuffers(uint32_t modelId, EdenBuffer** buffers, int32_t* numOfBuffers);
...
*/
RtRet GetMatchedEMABuffers(uint32_t modelId, EdenBuffer* buffers, void** emaBuffers, int32_t* numOfEmaBuffers = nullptr);
 ...
 *  @returns return code
 */
RtRet GetCompileVersion(uint32_t modelId, EdenModelFile* modelFile, char versions[][VERSION_LENGTH_MAX]);
...
/**
 *  @Dump Eden Info to Bugreport of Dumpstate
 *  @Dump Eden Info to Bugreport of Dumpstate
 *  @param[in] fd.
 *  @param[out] None
 *  @returns return code
 */
RtRet dumpAll(int fd);
