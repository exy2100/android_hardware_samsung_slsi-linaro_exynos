...
/*
 * @struct EdenMemoryInfo
 * @brief Structure used inside the Framework
 *        for memory management
 * @remark When allocating a buffer, the size and number are
 *         using the parameters reveived or nnc, tlife parsing.
 *         And the actual alllocation is done in emabuffer, and
 *         the buffer buffer information to be sent to the user
 *         uses Edenbuffer.
 */
typedef struct __EdenMemoryInfo {
  int32_t numOfBuffers;         // < number of buffer for EdenBuffer.
  EdenBuffer* buffers;          // < buffer list passed to the user.
  int32_t* sizeOfBuffers;
  int32_t* operandIds;
  int32_t* allocSizeOfBuffers;
  int32_t numOfEmaBuffers;      // < number of buffer for emaBuffer.
  eden_memory_t* emaBuffers;    // < list of buffers actually allocated.
} EdenMemoryInfo;
...
class EdenMemoryManager {
 public:
   ...
   * @returns return code
   */
  NnRet AllocateBuffersForOutputOperands(uint32_t modelId, EdenMemType memType, EdenBuffer** buffers,
   ...
   * @returns return code
   */
  NnRet LoadBuffersForInputOperands(uint32_t modelId, EdenMemType memType, UserBuffer* userBuffers, int32_t numOfBuffers, EdenBuffer** edenBuffers);
   ...
   * @returns return code
   */
  NnRet LoadBuffersForOutputOperands(uint32_t modelId, EdenMemType memType, UserBuffer* userBuffers,
   ...
   * @returns return code
   */
  NnRet AllocateBuffersForOperands(uint32_t modelId, EdenMemType memType, int32_t numOfOperand, int32_t* operandIds, EdenBuffer** buffers);
   ...
   * @returns return code
   */
  NnRet AllocateBuffersForOperand(uint32_t modelId, EdenMemType memType, int32_t operandId, EdenBuffer** buffers);

  NnRet AllocateBuffersForBridgeOperand(uint32_t modelId, addr_t refer, EdenMemType memType, int32_t operandId, EdenBuffer** buffers);
  NnRet AllocateBuffersForBindingBridgeOperand(uint32_t modelId, addr_t refer, EdenMemType memType,
                                               std::vector<int32_t> ncpOutputs, std::vector<int32_t> subtaskOutputs,
                                               EdenBuffer** buffers);
  NnRet GetBuffersForBridgeOperand(uint32_t modelId, addr_t refer, int32_t operandId, EdenBuffer** buffers, bool check = false);

  // Allocate Bridge buffers for custom operations (DSP)
  NnRet IsAlreadyAllocatedBridgeOperand(uint32_t modelId, addr_t refer, int32_t operandId);

  /**
   * @brief Allocated Shared buffer , which is used between npu and dsp.
   * @details This function allocates memory for shared buffer operand.
   *          Size of buffer is written in the width of operand.
   *          register the memory info to the map to release.
   */
  NnRet AllocateBuffersForSharedMem(EdenModel* edenModel, EdenMemType memType,
                                    EdenBuffer** buffers, int32_t* numOfBuffers);

  /**
   * @brief Get Shared buffer information for a given operandId from the given model.
   * @details This function finds a matched Shared buffer allocated before,
   *          from the Map modelId to SharedMemInfo.
   *          If fail to find, it will dump the memroy Infos for a given model id.
   */
  NnRet GetMatchedSharedBuffer(uint32_t modelId, int32_t operandId, eden_memory_t* emaBuffers);

  /**
   * @brief Get the number of shared buffer.
   * @details This function get the number of shared buffer for the given model.
   *          currently, only support 1 shared buffer, therefore it will return
   *          1 if there is shared buffer tensor, if it returns 0, it means there
   *          is no shared buffer tensor in the model.
   */
  NnRet GetNumOfSharedBuffer(uint32_t modelId, int32_t* numOfSharedBuffer);
   ...
   * @param[in] isFP16CFU true if FP16 CFU model (Pamir)
   * @returns Size for Z-order data
   */
  int32_t SizeForZOrder(int32_t width, int32_t height, int32_t channel, int32_t numOfBuffers,
                        uint32_t alignWidth, uint32_t alignHeight, uint32_t alignChannel,
                        bool isFP16CFU = false);
  ...
  NnRet GetSizeOfBufferForOperandId(uint32_t modelId, int32_t operandId, int32_t& sizeOfBuffer);
 ...
 private:
   ...
   * @returns return code
   */
  NnRet AllocateBuffers(int32_t numOfBuffers, int32_t* sizeOfBuffers, EdenMemType memType,
                        EdenBuffer** buffers, eden_memory_t** emaBuffers, int32_t numOfSharedBuffer = 0);

  /**
   * @brief Load buffers using given information
   * @details This function connects the user's ion buffer to the edenMemoryManager.
   * @param[in] numOfBuffers number of buffers to be allocated
   * @param[in] sizeOfBuffers array for each buffer size
   * @param[in] memType Memory type for input operands.
   * @param[in] buffers Array of UserBuffer*.
   ...
   * @returns return code
   */
  NnRet LoadBuffers(int32_t numOfBuffers, int32_t* sizeOfBuffers, EdenMemType memType, UserBuffer* userBuffers, EdenBuffer** edenBuffers, eden_memory_t** emaBuffers);
  ...
  /**
   * @brief Get Shared buffer size for a given operandId from the given model.
   * @details This function finds a matched Shared buffer operand from Map to Operand.
   *          return the size of buffer with data type and N, C, H, W.
   */
  NnRet GetSizeOfBufferForSharedMemory(EdenModel* model, int32_t operandId, int32_t& sizeOfBuffer);
  ...
  NnRet AllocateBindingBuffer(int32_t numOfBuffers, int32_t* sizeOfBuffers, EdenMemType memType,
                            EdenBuffer** buffers, eden_memory_t** emaBuffers);
  ...
  Mutex mutex_mapModelIdToSharedMemoryInfo_;

  eden::util::map<uint32_t, EdenModel*> mapModelIdToModel_;  //*< Map for (modelId, EdenModel*)

  // For Input/Output Buffer
  std::map<uint32_t, std::vector<EdenMemoryInfo*>> mapModelIdToInputMemoryInfo_ GUARDED_BY(mutex_mapModelIdToInputMemoryInfo_);     //*< Map for (modelId, input buffers)
  std::map<uint32_t, std::vector<EdenMemoryInfo*>> mapModelIdToOutputMemoryInfo_ GUARDED_BY(mutex_mapModelIdToOutputMemoryInfo_);   //*< Map for (modelId, output buffers)

  // For Shared Buffer( Buffer used between NPU and DSP )
  std::map<uint32_t, std::vector<EdenMemoryInfo*>> mapModelIdToSharedMemoryInfo_\
                                  GUARDED_BY(mutex_mapModelIdToSharedMemoryInfo_);

  // For Reallocate Buffer
  std::map<addr_t, std::map<int32_t, EdenBuffer*>> mapRequestIdToReallocatedBufferInfo_ GUARDED_BY(mutex_mapRequestIdToReallocatedBufferInfo_);  //*< Map for (requestId, map<operandId, EdenBuffer*>)
  std::map<uint32_t, std::vector<addr_t>> mapModelIdToRellocatedRequest_ GUARDED_BY(mutex_mapModelIdToRellocatedRequest_);                       //*< Map for (modelId, vector<requestId>)

  // For Bridge(Temp) Buffer
  std::map<addr_t, std::map<int32_t, EdenMemoryInfo*>> mapBufferAddrToBridgeBufferInfo_ GUARDED_BY(mutex_mapBufferAddrToBridgeBufferInfo_);   //*< Map for (inputBuffer addr, map<operandId, EdenMemoryInfo*>)
  std::map<uint32_t, std::vector<addr_t>> mapModelIdToBridgeBufferAddr_ GUARDED_BY(mutex_mapModelIdToBridgeBufferAddr_);                      //*< Map for (modelId, vector<inputBuffer addr>)
  std::map<uint32_t, std::vector<EdenMemoryInfo*>> mapModelIdToTempMemoryInfo_ GUARDED_BY(mutex_mapModelIdToTempMemoryInfo_);                 //*< Map for (modelId, vector<EdenMemoryInfo>)
  std::map<uint32_t, ModelPreference> mapModelIdToModelPreference_ GUARDED_BY(mutex_mapModelIdToModelPreference_);                            //*< Map for (modelId, ModelPreference)
};
