...
class ManagedBuffer {
 public:
    static std::shared_ptr<ManagedBuffer> create(EdenBuffer* buffer,
                                                 uint32_t size,
                                                 std::set<HalPreparedModelRole> roles,
                                                 const V1_3::Operand& operand);
    ManagedBuffer(EdenBuffer* buffer, uint32_t size,
                    std::set<HalPreparedModelRole> roles, const V1_3::Operand& operand);
    ...
    V1_3::ErrorStatus validateCopyFrom(const std::vector<uint32_t>& dimensions, uint32_t size) const;
    V1_3::ErrorStatus validateCopyTo(uint32_t size) const;

    EdenBuffer* getBuffer() const;
 ...
 private:
    ...
    const V1_3::OperandType kOperandType;
