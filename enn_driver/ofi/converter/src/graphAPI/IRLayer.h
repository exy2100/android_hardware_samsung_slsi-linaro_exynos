...
template <typename T> std::string &operator<<(std::string &&src, T i) {
    std::stringstream oss;
    oss << i;
    return src.append(oss.str());
}

template <typename T> std::string operator<<(const std::string &src, T i) {
    std::stringstream oss;
    oss << src;
    oss << i;
    return oss.str();
...
#define IR_ASSERT(x)                                                                                                   \
    if (!(x))                                                                                                          \
    THROW_IE_EXCEPTION << "Assert failed for " #x << ": "
...
struct BuilderContext {
    int layer_count;
    InferenceEngine::Precision layer_precision;
};
...
template <typename T> IRBlob::Ptr readBlobFromFile(const std::string &file) {
    auto fs = FileUtils::fileSize(file);
    if (fs <= 0)
        THROW("blob file ") << file << " not found or empty";
    InferenceEngine::Precision precision =
        sizeof(T) == sizeof(short) ? InferenceEngine::Precision::FP16 : InferenceEngine::Precision::FP32;
    auto ret = typename InferenceEngine::TBlob<T>::Ptr(
        new InferenceEngine::TBlob<T>(precision, InferenceEngine::C, {static_cast<size_t>(fs / sizeof(T))}));
    ret->allocate();
