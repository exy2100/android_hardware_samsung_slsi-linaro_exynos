...
inline OutputPort addOutput(const IRLayer &layer, const InferenceEngine::SizeVector &dims) {
    ...
    if (dims.size() == 2) {
        ...
        InferenceEngine::TensorDesc td(layer->precision, dims, InferenceEngine::Layout::NC);
        data = std::make_shared<InferenceEngine::Data>(d_name, td);

    } else if (dims.size() == 4) {
        std::cout << "addOutput dims size " << dims.size() << std::endl;
        // InferenceEngine::TensorDesc td(layer->precision, dims, InferenceEngine::Layout::ANY);
        InferenceEngine::TensorDesc td(layer->precision, dims, InferenceEngine::Layout::NCHW);
        // InferenceEngine::TensorDesc td(layer->precision, dims, InferenceEngine::Layout::NHWC);
        data = std::make_shared<InferenceEngine::Data>(d_name, td);

    } else if (dims.size() == 1) {
        std::cout << "addOutput dims size " << dims.size() << std::endl;
        // InferenceEngine::TensorDesc td(layer->precision, dims, InferenceEngine::Layout::ANY);
        InferenceEngine::TensorDesc td(layer->precision, dims, InferenceEngine::Layout::C);
        data = std::make_shared<InferenceEngine::Data>(d_name, td);
    } else {
        std::cout << "addOutput dims size " << dims.size() << std::endl;
        InferenceEngine::TensorDesc td(layer->precision, dims, InferenceEngine::Layout::ANY);
...
template <typename T> void addAttr(IRLayer layer, const std::string &a_name, T val) {
    std::stringstream oss;
    oss << val;
    layer->params[a_name] = oss.str();
};
template <typename T, typename S> std::shared_ptr<T> As(const std::shared_ptr<S> &src) {
...
inline IRLayer Generic(const std::string &type, BuilderContext &ctx) {
    std::string name = type + "-";  // todo: make it unique
    name = name << ctx.layer_count++;
    InferenceEngine::LayerParams prms;
    prms.precision = ctx.layer_precision;
    prms.name = name;
    prms.type = type;
    return std::make_shared<InferenceEngine::CNNLayer>(prms);
}

inline IRLayer Generic(const std::string &type, const OutputPort &src, BuilderContext &ctx) {
    std::string name = type + "-";  // todo: make it unique
    name = name << ctx.layer_count++;
    InferenceEngine::LayerParams prms;
    prms.precision = ctx.layer_precision;
...
inline IRLayer Generic(const std::string &type, const IRLayer &src, BuilderContext &ctx) {
    return Generic(type, output(src), ctx);
}

template <typename T, typename A> std::string dumpVec(std::vector<T, A> const &vec) {
    if (vec.empty())
        return "[]";
    std::stringstream oss;
    oss << "[" << vec[0];
...
namespace FCLayer {
static IRLayer create(const IRBlob::Ptr &weights, const OutputPort &src, BuilderContext &ctx) {
#ifdef NNLOG
    ...
    prm.precision = ctx.layer_precision;
    prm.name = name;
    ...
    if (wDim.size() == 2) {
        ...
        ALOGI("wDims[0] = %d wDim[1] = %d", wDim[0], wDim[1]);
#endif
...
static OutputPort
ScaleShiftNode(const OutputPort &src, const IRBlob::Ptr &scale, const IRBlob::Ptr &bias, BuilderContext &ctx) {
...
inline OutputPort AddTryConst(const OutputPort &src, const IRBlob::Ptr &biases, BuilderContext &ctx) {
    ...
    if (fc) {
        ...
        return ScaleShiftNode(src, nullptr, biases, ctx);
    }
}

#if 0
inline OutputPort operator+(const OutputPort &src, const IRBlob::Ptr &biases) {
    return AddTryConst(src, biases, ctx);
}
#endif
/*
inline OutputPort operator+(const OutputPort &src, const IRBlob::Ptr &biases)
...
namespace ConvLayer {
static IRLayer create(const OutputPort &src, BuilderContext &ctx) {
...
struct AsymmQuantParams {
    float scale;
    int32_t zeroPoint;
};
struct QuantizationParams {
    std::vector<AsymmQuantParams> inputs;
    std::vector<AsymmQuantParams> outputs;
    AsymmQuantParams weight;
    AsymmQuantParams bias;
};
struct ConvolutionParams {
    int groups = 1;
    Point2D kernel, stride = {1, 1}, pad_start = {0, 0}, pad_end = {0, 0}, dilations = {1, 1};
    int num_output_planes;
    IRBlob::Ptr weights;
    IRBlob::Ptr biases;
    std::string padType;
    InferenceEngine::Layout layout = InferenceEngine::Layout::NHWC;
};
...
inline OutputPort
Convolution(const OutputPort &src, const ConvolutionParams &prms, QuantizationParams &qtprms, BuilderContext &ctx) {
    auto ret = As<InferenceEngine::ConvolutionLayer>(ConvLayer::create(src, ctx));
    ...
    ret->_dilation.clear();
    ret->_dilation.insert(InferenceEngine::X_AXIS, prms.dilations.x);
    ret->_dilation.insert(InferenceEngine::Y_AXIS, prms.dilations.y);

    ret->_group = prms.groups;
    ret->_out_depth = prms.num_output_planes;

    //<data dilation-x="1" dilation-y="1" group="1" kernel-x="3" kernel-y="3" output="8" pad-x="0" pad-y="0"
    // stride="1,1,2,2" stride-x="2" stride-y="2"/>

    ret->params["auto_pad"] = prms.padType;
    ret->params["dilations"] = std::to_string(ret->_dilation.at(InferenceEngine::Y_AXIS)) + "," +
                               std::to_string(ret->_dilation.at(InferenceEngine::X_AXIS));
    ret->params["group"] = std::to_string(ret->_group);

    ret->params["kernel"] = std::to_string(ret->_kernel.at(InferenceEngine::Y_AXIS)) + "," +
                            std::to_string(ret->_kernel.at(InferenceEngine::X_AXIS));
    ret->params["output"] = std::to_string(ret->_out_depth);
    ret->params["pads_begin"] = std::to_string(ret->_padding.at(InferenceEngine::Y_AXIS)) + "," +
                                std::to_string(ret->_padding.at(InferenceEngine::X_AXIS));
    ret->params["pads_end"] = std::to_string(ret->_pads_end.at(InferenceEngine::Y_AXIS)) + "," +
                              std::to_string(ret->_pads_end.at(InferenceEngine::X_AXIS));
    ret->params["strides"] = std::to_string(ret->_stride.at(InferenceEngine::Y_AXIS)) + "," +
                             std::to_string(ret->_stride.at(InferenceEngine::X_AXIS));
    if (qtprms.inputs.size() == 1) {
        ret->params["scale_in_0"] = float_to_string(qtprms.inputs[0].scale);
        ret->params["zeropoint_in_0"] = std::to_string(qtprms.inputs[0].zeroPoint);
    } else {
#ifdef NNLOG
        ALOGE("Convolution needs 1 input quantization param.");
#endif
    }
    if (qtprms.outputs.size() == 1) {
        ret->params["scale_out_0"] = float_to_string(qtprms.outputs[0].scale);
        ret->params["zeropoint_out_0"] = std::to_string(qtprms.outputs[0].zeroPoint);
    } else {
#ifdef NNLOG
        ALOGE("Convolution needs 1 output quantization param.");
#endif
    }
    ret->params["scale_weight"] = float_to_string(qtprms.weight.scale);
    ret->params["zeropoint_weight"] = std::to_string(qtprms.weight.zeroPoint);
    ret->params["scale_bias"] = float_to_string(qtprms.bias.scale);
    ret->params["zeropoint_bias"] = std::to_string(qtprms.bias.zeroPoint);

    ret->params["layout"] = (prms.layout == InferenceEngine::Layout::NCHW) ? "NCHW" : "NHWC";
    ...
    ALOGI(
        ...
        prms.groups,
        ret->_kernel.at(InferenceEngine::X_AXIS),
        ret->_kernel.at(InferenceEngine::Y_AXIS),
        ret->_stride.at(InferenceEngine::X_AXIS),
        ret->_stride.at(InferenceEngine::Y_AXIS),
        ret->_padding.at(InferenceEngine::X_AXIS),
        ret->_padding.at(InferenceEngine::Y_AXIS),
        ret->_pads_end.at(InferenceEngine::X_AXIS),
    ...
    if (prms.padType == "explicit") {
        ...
        // addOutput(ret, {inDims[0], (size_t) out_size.y, (size_t) out_size.x, (size_t) prms.num_output_planes});
        // //nhwc
    } else {
        ...
        //   size_t PH = ret->_padding[InferenceEngine::Y_AXIS];  // comment to avoid unused warning
        ...
        } else /*if (prms.padType == "same_lower")*/ {  // comment to avoid uninitialized warning
...
inline OutputPort FullyConnected(const OutputPort &src,
                                 const IRBlob::Ptr &weights,
                                 const IRBlob::Ptr &biases,
                                 QuantizationParams &qtprms,
                                 BuilderContext &ctx) {
    auto ret = As<InferenceEngine::FullyConnectedLayer>(FCLayer::create(weights, src, ctx));
    if (qtprms.inputs.size() == 1) {
        ret->params["scale_in_0"] = float_to_string(qtprms.inputs[0].scale);
        ret->params["zeropoint_in_0"] = std::to_string(qtprms.inputs[0].zeroPoint);
    } else {
#ifdef NNLOG
        ALOGE("FullyConnected needs 1 input quantization param.");
#endif
    }
    if (qtprms.outputs.size() == 1) {
        ret->params["scale_out_0"] = float_to_string(qtprms.outputs[0].scale);
...
#ifdef NNLOG
        ALOGE("FullyConnected needs 1 output quantization param.");
#endif
    }
    ret->params["scale_weight"] = float_to_string(qtprms.weight.scale);
    ret->params["zeropoint_weight"] = std::to_string(qtprms.weight.zeroPoint);
    ret->params["scale_bias"] = float_to_string(qtprms.bias.scale);
    ret->params["zeropoint_bias"] = std::to_string(qtprms.bias.zeroPoint);
    return AddTryConst(output(ret), biases, ctx);
}
...
inline IRLayer BatchNormalization(const OutputPort &src, BatchNormParams &prms, BuilderContext &ctx) {
    auto inp = src;
...
inline OutputPort LRN(const OutputPort &src,
                      float alpha,
                      float beta,
                      int local_size,
                      BuilderContext &ctx,
...
inline OutputPort Pooling(const OutputPort &inp,
    ...
    */
    ...
    ret->_stride.clear();
    ret->_stride.insert(InferenceEngine::X_AXIS, stride_copy.x);
    ret->_stride.insert(InferenceEngine::Y_AXIS, stride_copy.y);
    ret->_padding.clear();
    ...
    Point2D out_size = (in_size + pad + pad - kernel + stride_copy) / stride_copy;
    src >> ret;
...
#ifdef NNLOG
        ALOGE("Concat needs at least 1 input quantization param.");
#endif
    }
    if (qtprms.outputs.size() == 1) {
        ret->params["scale_out_0"] = float_to_string(qtprms.outputs[0].scale);
...
#ifdef NNLOG
        ALOGE("Concat needs 1 output quantization param.");
#endif
    }
    ret->params["layout"] = (layout == InferenceEngine::Layout::NCHW) ? "NCHW" : "NHWC";
...
//        pad_end.x= %d pad_end.y= %d ", kernel.x, kernel.y, stride.x, stride.y, pad_start.x, pad_start.y, pad_end.x, pad_end.y);
#endif
    //<data exclude-pad="true" kernel-x="4" kernel-y="4" pad-x="0" pad-y="0" pool-method="avg" stride="1,1,2,2"
    // stride-x="2" stride-y="2"/>
    ret->params["auto_pad"] = padType;
    ret->params["exclude-pad"] = ret->_exclude_pad ? "true" : "false";
    ret->params["kernel"] = std::to_string(ret->_kernel.at(InferenceEngine::Y_AXIS)) + "," +
                            std::to_string(ret->_kernel.at(InferenceEngine::X_AXIS));
    ret->params["pads_begin"] = std::to_string(ret->_padding.at(InferenceEngine::Y_AXIS)) + "," +
                                std::to_string(ret->_padding.at(InferenceEngine::X_AXIS));
    ...
    if (padType == "explicit") {
        Point2D in_size = {static_cast<int>(inDims[3]), static_cast<int>(inDims[2])};
        // todo: handle uneven padding
        Point2D out_size = (in_size - kernel + pad_start + pad_end + stride_copy) / stride_copy;  // add stride-1 to round ceiling
        ...
        size_t SH = ret->_stride[InferenceEngine::Y_AXIS];
        size_t SW = ret->_stride[InferenceEngine::X_AXIS];
        //   size_t PH = ret->_padding[InferenceEngine::Y_AXIS]; // commented to avoid unused warning
        //   size_t PW = ret->_padding[InferenceEngine::X_AXIS];
...
namespace SumLayer {
static IRLayer create(const OutputPort &src1, const OutputPort &src2, BuilderContext &ctx) {
...
namespace MulLayer {
static IRLayer create(const OutputPort &src1, const OutputPort &src2, QuantizationParams &qtprms, BuilderContext &ctx) {
    ...
    if (qtprms.inputs.size() == 2) {
        for (size_t idx = 0; idx < qtprms.inputs.size(); idx++) {
            std::string scaleParam = "scale_in_" + std::to_string(idx);
            std::string zeroPointParam = "zeropoint_in_" + std::to_string(idx);
            mul->params[scaleParam] = float_to_string(qtprms.inputs[idx].scale);
            mul->params[zeroPointParam] = std::to_string(qtprms.inputs[idx].zeroPoint);
        }
    } else {
#ifdef NNLOG
        ALOGE("Mul needs 2 input quantization params.");
#endif
    }
    if (qtprms.outputs.size() == 1) {
        mul->params["scale_out_0"] = float_to_string(qtprms.outputs[0].scale);
        mul->params["zeropoint_out_0"] = std::to_string(qtprms.outputs[0].zeroPoint);
    } else {
#ifdef NNLOG
        ALOGE("Mul needs 1 output quantization param.");
#endif
    }
    src1 >> mul;
    src2 >> mul;
...
inline OutputPort Mul(const OutputPort &a, const OutputPort &b, QuantizationParams &qtprms, BuilderContext &ctx) {
    return output(MulLayer::create(a, b, qtprms, ctx));
}
#if 0
...
static OutputPort Diagnoal(const Vector &weights, const OutputPort &src, BuilderContext& ctx)
{
    std::string name = "ConstMul-"; // todo: make it unique
    name = name << ctx.layer_count++;
    InferenceEngine::LayerParams prm;
...
extern const std::string ReLU1;

extern const std::string ReLU6;

static inline IRLayer
create(const OutputPort &src, const std::string &type, QuantizationParams &qtprms, BuilderContext &ctx) {
    std::string name = type + "-";  // todo: make it unique
    name = name << ctx.layer_count++;
    IRLayer layer;
    if ((strncasecmp(type.c_str(), "relu", type.size()) == 0)) {
        ...
        layer->type = "ReLU";
    } else if ((strncasecmp(type.c_str(), "relu1", type.size()) == 0)) {
        InferenceEngine::LayerParams prm;
        prm.precision = ctx.layer_precision;
        prm.name = name;
        layer = std::make_shared<InferenceEngine::ReLULayer>(prm);
        layer->type = "ReLU1";
    } else if ((strncasecmp(type.c_str(), "relu6", type.size()) == 0)) {
        InferenceEngine::LayerParams prm;
        prm.precision = ctx.layer_precision;
        prm.name = name;
        layer = std::make_shared<InferenceEngine::ReLULayer>(prm);
    ...
    if (qtprms.inputs.size() == 1) {
        layer->params["scale_in_0"] = float_to_string(qtprms.inputs[0].scale);
        layer->params["zeropoint_in_0"] = std::to_string(qtprms.inputs[0].zeroPoint);
    } else {
#ifdef NNLOG
        ALOGE("ActivationLayer needs 1 input quantization param.");
#endif
    }
    if (qtprms.outputs.size() == 1) {
        layer->params["scale_out_0"] = float_to_string(qtprms.outputs[0].scale);
        layer->params["zeropoint_out_0"] = std::to_string(qtprms.outputs[0].zeroPoint);
    } else {
#ifdef NNLOG
        ALOGE("ActivationLayer needs 1 output quantization param.");
#endif
    }
...
template <typename T> OutputPort ReLU(const T &src, QuantizationParams &qtprms, BuilderContext &ctx) {
    return output(ActivationLayer::create(src, ActivationLayer::ReLU, qtprms, ctx));
}

template <typename T> OutputPort ReLU1(const T &src, QuantizationParams &qtprms, BuilderContext &ctx) {
    return output(ActivationLayer::create(src, ActivationLayer::ReLU1, qtprms, ctx));
}

template <typename T> OutputPort ReLU6(const T &src, QuantizationParams &qtprms, BuilderContext &ctx) {
    return output(ActivationLayer::create(src, ActivationLayer::ReLU6, qtprms, ctx));
}

template <typename T> OutputPort Sigmoid(const T &src, QuantizationParams &qtprms, BuilderContext &ctx) {
    return output(ActivationLayer::create(src, ActivationLayer::Sigmoid, qtprms, ctx));
}

template <typename T> OutputPort Tanh(const T &src, QuantizationParams &qtprms, BuilderContext &ctx) {
    return output(ActivationLayer::create(src, ActivationLayer::Tanh, qtprms, ctx));
}

namespace SplitUtil {

static IRLayer create(int size, const OutputPort &src, BuilderContext &ctx, int axis = 1) {
...
inline std::vector<OutputPort> Split(const OutputPort &src, int splitElements, BuilderContext &ctx, int axis = 1) {
    return SplitUtil::create(splitElements, src, ctx, axis)->outData;
}

inline std::vector<OutputPort> Split(const IRLayer &src, int splitElements, BuilderContext &ctx, int axis = 1) {
    return Split(output(src), splitElements, ctx, axis);
}

inline OutputPort
Concat(const std::vector<OutputPort> inputs, QuantizationParams &qtprms, BuilderContext &ctx, int axis = 1) {
    ...
    if (qtprms.inputs.size() >= 1) {
        for (size_t idx = 0; idx < qtprms.inputs.size(); idx++) {
            std::string scaleParam = "scale_in_" + std::to_string(idx);
            std::string zeroPointParam = "zeropoint_in_" + std::to_string(idx);
            ret->params[scaleParam] = float_to_string(qtprms.inputs[idx].scale);
            ret->params[zeroPointParam] = std::to_string(qtprms.inputs[idx].zeroPoint);
        }
    } else {
#ifdef NNLOG
        ALOGE("Concat needs at least 1 input quantization param.");
...
// template<typename T>
inline OutputPort Clamp(const OutputPort &src, float min, float max, QuantizationParams &qtprms, BuilderContext &ctx) {
...
#ifdef NNLOG
        ALOGE("Clamp  needs 1 input quantization param.");
#endif
    }
    if (qtprms.outputs.size() == 1) {
        layer->params["scale_out_0"] = float_to_string(qtprms.outputs[0].scale);
...
#ifdef NNLOG
        ALOGE("Clamp needs 1 output quantization param.");
#endif
    }
...
inline OutputPort L2Normalization(const OutputPort &src, bool isAcross, bool isShareChannel, BuilderContext &ctx) {
    auto layer = Generic("Normalize", src, ctx);
...
inline OutputPort
Reshape(const TensorDims &newDims, const OutputPort &src, QuantizationParams &qtprms, BuilderContext &ctx) {
...
inline OutputPort
Softmax(const OutputPort &src, float beta, int axis, QuantizationParams &qtprms, BuilderContext &ctx) {
    ...
    */
    ...
    l->type = "SoftMax";
    src >> l;
    l->params["axis"] = std::to_string(axis);
    l->params["beta"] = std::to_string(beta);
    // addOutput(l, src->getTensorDesc().getDims());

    if (qtprms.inputs.size() == 1) {
        l->params["scale_in_0"] = float_to_string(qtprms.inputs[0].scale);
        l->params["zeropoint_in_0"] = std::to_string(qtprms.inputs[0].zeroPoint);
    } else {
#ifdef NNLOG
        ALOGE("Softmax needs 1 input quantization param.");
#endif
    }
    if (qtprms.outputs.size() == 1) {
        l->params["scale_out_0"] = float_to_string(qtprms.outputs[0].scale);
        l->params["zeropoint_out_0"] = std::to_string(qtprms.outputs[0].zeroPoint);
    } else {
#ifdef NNLOG
        ALOGE("Softmax needs 1 output quantization param.");
#endif
    }
...
inline OutputPort Gather(const std::vector<OutputPort> inputs, BuilderContext &ctx, int axis = 1) {
...
inline OutputPort Add(const OutputPort &src1, const OutputPort &src2, QuantizationParams &qtprms, BuilderContext &ctx) {
    auto ret = As<InferenceEngine::EltwiseLayer>(SumLayer::create(src1, src2, ctx));
    if (qtprms.inputs.size() == 2) {
        for (size_t idx = 0; idx < qtprms.inputs.size(); idx++) {
            std::string scaleParam = "scale_in_" + std::to_string(idx);
            std::string zeroPointParam = "zeropoint_in_" + std::to_string(idx);
...
#ifdef NNLOG
        ALOGE("Sum needs 2 input quantization params.");
#endif
    }
    if (qtprms.outputs.size() == 1) {
        ret->params["scale_out_0"] = float_to_string(qtprms.outputs[0].scale);
...
#ifdef NNLOG
        ALOGE("Sum needs 1 output quantization param.");
#endif
    }
    return output(ret);
}
...
inline OutputPort AddConst(IRDocument &doc,
                           const OutputPort &src,
                           const IRBlob::Ptr &biases,
                           QuantizationParams &qtprms,
                           BuilderContext &ctx) {
    // use const layer with elment wise add
    auto constNode = Generic("Const", ctx);

    if (qtprms.inputs.size() == 2) {
        for (size_t idx = 0; idx < qtprms.inputs.size(); idx++) {
            std::string scaleParam = "scale_in_" + std::to_string(idx);
            std::string zeroPointParam = "zeropoint_in_" + std::to_string(idx);
            constNode->params[scaleParam] = float_to_string(qtprms.inputs[idx].scale);
            constNode->params[zeroPointParam] = std::to_string(qtprms.inputs[idx].zeroPoint);
        }
    } else {
#ifdef NNLOG
        ALOGE("Const Sum needs 2 input quantization params.");
#endif
    }
    if (qtprms.outputs.size() == 1) {
        constNode->params["scale_out_0"] = float_to_string(qtprms.outputs[0].scale);
        constNode->params["zeropoint_out_0"] = std::to_string(qtprms.outputs[0].zeroPoint);
    } else {
#ifdef NNLOG
        ALOGE("Const SUM needs 1 output quantization param.");
#endif
    }
    doc.add(constNode);
    constNode->blobs["custom"] = biases;
    const auto constOut = addOutput(constNode, src->getTensorDesc().getDims());
    return Add(src, constOut, qtprms, ctx);
}
