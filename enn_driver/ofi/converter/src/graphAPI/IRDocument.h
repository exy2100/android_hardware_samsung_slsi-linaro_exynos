...
using BlobPair = std::pair<InferenceEngine::TBlob<int8_t>::Ptr, InferenceEngine::TBlob<int8_t>::Ptr>;
namespace IRBuilder {
...
class IRDocument {
  private:
    ...
    std::unordered_map<std::string, size_t> _shuffledWeightSizeMap;
    std::unordered_map<std::string, size_t> _shuffledBiasSizeMap;
    InferenceEngine::Precision mPrecision = InferenceEngine::Precision::UNSPECIFIED;
    ...
    // save a layer
    void saveToIR(size_t &bin_size, pugi::xml_node &parent, const IRLayer &irLayer);
    // svae a blob
    void saveBlobToIR(size_t &bin_size,
                      const InferenceEngine::Blob::Ptr &blob,
                      pugi::xml_node &layer,
                      const std::string &name);

    void calcShuffledBlobSize();
    void saveShuffledBlobs(InferenceEngine::TBlob<int8_t>::Ptr weight, InferenceEngine::TBlob<int8_t>::Ptr bias);
    void ShuffleWeight(unsigned char *s_weight_ptr,
                       const unsigned char *weight_ptr,
                       int kernel_w,
                       int kernel_h,
                       int channel,
                       int kernel_n);
    void ShuffleWeight_PadtoC16(unsigned char *s_weight_ptr,
                                const unsigned char *weight_ptr,
                                int kernel_w,
                                int kernel_h,
                                int channel,
                                int kernel_n);
    void ShuffleWeight_DW(unsigned char *s_weight_ptr,
                          const unsigned char *weight_ptr,
                          int kernel_w,
                          int kernel_h,
                          int kernel_n);
    size_t padForAlign(unsigned char *out, size_t wrote, size_t block);
    size_t writeNameNLength(unsigned char *out, std::string name, int length);
  ...
  public:
    ...
    // Products
    void save(std::ostream &xml_os, size_t &bin_size);
    void crateDotFile(std::ostream &dot) const;
    BlobPair saveShuffledBlobs();
