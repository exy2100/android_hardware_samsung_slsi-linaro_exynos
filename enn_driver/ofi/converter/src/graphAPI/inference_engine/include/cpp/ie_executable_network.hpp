...
class ExecutableNetwork {
  ...
  public:
    ...
    */
    ...
    StatusCode Compile(CompileMode mode = FULL_FILE, const std::string& dsp_target = std::string("980"),
                       char** cgo_mem = nullptr, uint32_t *cgo_size = 0,
                       bool is_ofi = true) {
       if(actual->Compile(mode, dsp_target, cgo_mem, cgo_size,
                           is_ofi)) return StatusCode::OK;
        return StatusCode::GENERAL_ERROR;
    }

    void LoadKernelInfo(const std::string &kernelFileName) {
        actual->LoadKernelInfo(kernelFileName);
    }
    ...
    */
    CNNNetwork GetExecNNGraphInfo() {
        ICNNNetwork::Ptr ptr = nullptr;
        CALL_STATUS_FNC(GetExecNNGraphInfo, ptr);
    ...
    * @return CNNetwork containing Executable Graph Info
    */
    std::shared_ptr<seva::graph::Graph> GetExecCVGraphInfo() {
        std::shared_ptr<seva::graph::Graph> ptr = nullptr;
        CALL_STATUS_FNC(GetExecCVGraphInfo, ptr);
        return ptr;
    }
#endif
    /**
