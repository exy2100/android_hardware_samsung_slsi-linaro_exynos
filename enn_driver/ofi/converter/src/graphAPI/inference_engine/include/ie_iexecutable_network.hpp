...
class IExecutableNetwork : public details::IRelease {
  public:
    ...
    * @return Status code of the operation: OK (0) for success
    */
    virtual StatusCode GetExecNNGraphInfo(ICNNNetwork::Ptr &graphPtr, ResponseDesc *resp) noexcept = 0;
#ifdef ENABLE_CV_API
    virtual StatusCode GetExecCVGraphInfo(std::shared_ptr<seva::graph::Graph> &graphPtr, ResponseDesc *resp) noexcept = 0;
#endif
    virtual bool Compile(CompileMode mode = FULL_FILE, const std::string& dsp_target = std::string("980"),
                        char** cgo_mem = nullptr, uint32_t *cgo_size = 0,
                        bool is_ofi = true) = 0;

    virtual void LoadKernelInfo(const std::string &kernelFileName) = 0;
    /**
